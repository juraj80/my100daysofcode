...
def test_friends_teams_two_order_does_not_matter(test_input,expected):

    combos = friends_teams(friends, team_size=2, order_does_matter=False)
    assert len(combos) == 6
    if expected: # return True or False
...
def test_friends_teams_two_order_does_matter(test_input,expected):

    combos = friends_teams(friends, team_size=2, order_does_matter=True)
...
def test_team_of_three_order_does_not_matter(test_input, expected):
    combos = friends_teams(friends, team_size=3, order_does_matter=False)
    assert len(combos) == 4
    if expected:
        assert test_input in combos
    else:
...
def test_team_of_three_order_does_matter(test_input, expected):
    combos = friends_teams(friends, team_size=3, order_does_matter=True)
